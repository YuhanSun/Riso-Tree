...
/**
 * Instances of this class represent the primary layer of the OSM Dataset. It extends the
 * DynamicLayer class becauase the OSM dataset can have many layers. Only one is primary, the layer
 * containing all ways. Other layers are dynamic.
 * 
 * @author craig
 * @since 1.0.0
 */
public class OSMLayer extends DynamicLayer {
  ...
  /**
   * This method is used to find or construct the necessary dataset object on an existing dataset
   * node and layer. This will create the relationships between the two if it is missing.
   */
  ...
  /**
   * Provides a method for iterating over all nodes that represent geometries in this layer. This is
   * similar to the getAllNodes() methods from GraphDatabaseService but will only return nodes that
   * this dataset considers its own, and can be passed to the GeometryEncoder to generate a
   * Geometry. There is no restricting on a node belonging to multiple datasets, or multiple layers
   ...
   * </pre>
   * 
   * This will work with OSM datasets, traversing from the geometry node to the way node and then to
   * the tags node to test if the way is a residential street.
   */
  ...
  /**
   * Add a rule for a pure way based search, with a single property key/value match on the way tags.
   * All ways with the specified tag property will be returned. This convenience method will
   * automatically name the layer based on the key/value passed, namely 'key-value'. If you want
   * more control over the naming, revert to the addDynamicLayerOnWayTags method. The geometry is
   * assumed to be LineString, the most common type for ways.
   * 
   * @param key
   * @param value
   */
   ...
   * All ways with the specified tag property will be returned. This convenience method will
   * automatically name the layer based on the key/value passed, namely 'key-value'. If you want
   * more control over the naming, revert to the addDynamicLayerOnWayTags method.
   * 
   * @param key
   * @param value
  ...
  /**
   * Add a rule for a pure way based search, with multiple property key/value match on the way tags.
   * All ways with the specified tag properties will be returned. This convenience method will
   * automatically name the layer based on the key/value pairs passed, namely
   * 'key-value-key-value-...'. If you want more control over the naming, revert to the
   * addDynamicLayerOnWayTags method.
   * 
   * @param geometry type as defined in Constants.
   * @param query String of ',' separated key=value tags to match
   */
   ...
   * automatically name the layer based on the key/value pairs passed, namely
   * 'key-value-key-value-...'. If you want more control over the naming, revert to the
   * addDynamicLayerOnWayTags method. The geometry type will be assumed to be LineString.
   * 
   * @param query String of ',' separated key=value tags to match
   */
  ...
  public DynamicLayerConfig addSimpleDynamicLayer(int gtype) {
    return addDynamicLayerOnWayTags(SpatialDatabaseService.convertGeometryTypeToName(gtype), gtype,
        null);
  }

  /**
   * The OSM dataset has a number of possible stylesOverride this method to provide a style if your
   * layer wishes to control its own rendering in the GIS.
   * 
