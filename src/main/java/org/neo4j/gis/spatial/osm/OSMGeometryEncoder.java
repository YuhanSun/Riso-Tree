...
public class OSMGeometryEncoder extends AbstractGeometryEncoder {
  ...
  /**
   * This class allows for OSM to avoid having empty tags nodes when there are no properties on a
   * geometry.
   * 
   * @author craig
   */
  ...
  public void encodeEnvelope(Envelope mbb, PropertyContainer container) {
    container.setProperty(PROP_BBOX,
        new double[] {mbb.getMinX(), mbb.getMaxX(), mbb.getMinY(), mbb.getMaxY()});
  }
  ...
  /**
   * This wrapper class allows the traverser to run simply down the NEXT chain, but we wrap this to
   * return the --NODE-->(node) results instead of the proxy nodes.
   * 
   * @author craig
   */
  private class NodeProxyIterator implements Iterator<Node> {
    ...
    NodeProxyIterator(Node first) {
      TraversalDescription traversalDescription = first.getGraphDatabase().traversalDescription()
    ...
    public Node next() {
      return traverser.next().endNode().getSingleRelationship(OSMRelation.NODE, Direction.OUTGOING)
          .getEndNode();
    }
  ...
  public Iterable<Node> getPointNodesFromWayNode(Node wayNode) {
    final Node firstNode =
        wayNode.getSingleRelationship(OSMRelation.FIRST_NODE, Direction.OUTGOING).getEndNode();
  ...
  public Geometry decodeGeometry(PropertyContainer container) {
    ...
    try {
      ...
      if (osmNode.hasProperty("node_osm_id")) {
        return geomFactory.createPoint(new Coordinate((Double) osmNode.getProperty("lon", 0.0),
            (Double) osmNode.getProperty("lat", 0.0)));
  ...
  private Geometry decodeGeometryFromRelation(Node osmNode, int gtype,
  ...
  /**
   * Since OSM users can construct any weird combinations of geometries, we need general code to
   * make the best guess. This method will find a enclosing LinearRing around any geometry except
   * Point and a straight LineString, and return that. For sensible types, it returns a more
  ...
  private Geometry decodeGeometryFromWay(Node wayNode, int gtype, int vertices,
      GeometryFactory geomFactory) {
    ...
    for (Node node : getPointNodesFromWayNode(wayNode)) {
      ...
      coordinates
          .add(new Coordinate((Double) node.getProperty("lon"), (Double) node.getProperty("lat")));
    }
    decodedCount++;
    if (overrun) {
      System.out.println("Overran expected number of way nodes: " + wayNode + " (" + overrunCount
          + "/" + decodedCount + ")");
    }
    if (coordinates.size() != vertices) {
      if (vertexMistmaches++ < 10) {
        System.err.println("Mismatching vertices size for "
            + SpatialDatabaseService.convertGeometryTypeToName(gtype) + ":" + wayNode + ": "
            + coordinates.size() + " != " + vertices);
    ...
    switch (coordinates.size()) {
      ...
      default:
        switch (gtype) {
          ...
          case GTYPE_POLYGON:
            return geomFactory.createPolygon(geomFactory.createLinearRing(
                coordinates.toArray(new Coordinate[coordinates.size()])), new LinearRing[0]);
          default:
            return geomFactory
  ...
  @Override
  /**
   * For OSM data we can build basic geometry shapes as sub-graphs. This code should produce the
   * same kinds of structures that the utilities in the OSMDataset create. However those structures
   * are created from original OSM data, while here we attempt to create equivalent graphs from JTS
   * Geometries. Note that this code is unable to connect the resulting sub-graph into the OSM data
   * model, since the only node it has is the geometry node. Those connections to the rest of the
   * OSM model need to be done in OSMDataset.
   */
  ...
  private class CombinedAttributes {
    ...
    CombinedAttributes(Node geomNode) {
      try {
        ...
        Node changeset =
            node.getSingleRelationship(OSMRelation.CHANGESET, Direction.OUTGOING).getEndNode();
        if (changeset != null) {
          extra.put("changeset", changeset.getProperty("changeset", null));
          Node user =
              changeset.getSingleRelationship(OSMRelation.USER, Direction.OUTGOING).getEndNode();
          if (user != null) {
    ...
    public Object getProperty(String key) {
      return extra.containsKey(key) ? extra.get(key)
          : node.hasProperty(key) ? node.getProperty(key, null) : properties.getProperty(key, null);
    }
  ...
  /**
   * This method wraps the hasProperty(String) method on the geometry node. This means the default
   * way of storing attributes is simply as properties of the geometry node. This behaviour can be
   * changed by other domain models with different encodings.
   * 
   * @param geomNode
   * @param name to test
   * @return
  ...
  /**
   * This method wraps the getProperty(String,null) method on the geometry node. This means the
   * default way of storing attributes is simply as properties of the geometry node. This behaviour
   * can be changed by other domain models with different encodings. If the property does not exist,
