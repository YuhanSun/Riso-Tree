...
public class SpatialDatabaseRecord implements Constants, SpatialRecord {
  ...
  /**
   * This method returns a simple integer representation of the geometry. Some geometry encoders
   * store this directly as a property of the geometry node, while others might store this
   * information elsewhere in the graph, or deduce it from other factors of the data model. See the
   * GeometryEncoder for information about mapping from the data model to the geometry.
   * 
   * @return integer representation of a geometry
   * @deprecated This method is of questionable value, since it is better to query the geometry
   *             object directly, outside the result
   */
  public int getType() {
  ...
  /**
   * Not all geometry records have the same attribute set, so we should test for each specific
   * record if it contains that property.
   * 
   * @param name
   * @return
   */
  ...
  public Object getProperty(String name) {
    PropertyMapper mapper = layer.getPropertyMappingManager().getPropertyMapper(name);
    return mapper == null ? getGeometryProperty(name)
        : mapper.map(getGeometryProperty(mapper.from()));
  }
  ...
  public String toString() {
    return "SpatialDatabaseRecord[" + getNodeId() + "]: type='" + getType() + "', props["
        + getPropString() + "]";
  }
