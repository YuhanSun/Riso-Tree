...
/**
 * Simple encoder that stores geometries as an linked list of point nodes. Only supports LineString
 * geometries.
 * 
 * @TODO: Consider generalizing this code and making a general linked list geometry store available
 *        in the library
 * @author craig
 */
public class SimpleGraphEncoder extends AbstractGeometryEncoder {
  ...
  public Geometry decodeGeometry(PropertyContainer container) {
    ...
    for (Node point : td.traverse(node).nodes()) {
      coordinates.add(new Coordinate((Double) point.getProperty("x"),
          (Double) point.getProperty("y"), (Double) point.getProperty("z")), false);
    }
