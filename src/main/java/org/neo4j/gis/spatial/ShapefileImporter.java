...
public class ShapefileImporter implements Constants {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (args.length < 2 || args.length > 4) {
      throw new IllegalArgumentException(
          "Parameters: neo4jDirectory shapefile [layerName commitInterval]");
    }
    ...
    GraphDatabaseService database =
        new GraphDatabaseFactory().newEmbeddedDatabase(new File(neoPath));
    try {
      ShapefileImporter importer =
          new ShapefileImporter(database, new NullListener(), commitInterval);
  ...
  public List<Node> importFile(String dataset, String layerName, Charset charset)
      throws IOException {
    Class<? extends Layer> layerClass =
        maintainGeometryOrder ? OrderedEditableLayer.class : EditableLayerImpl.class;
    EditableLayerImpl layer = (EditableLayerImpl) spatialDatabase.getOrCreateLayer(layerName,
        WKBGeometryEncoder.class, layerClass);
    return importFile(dataset, layer, charset);
  }

  public List<Node> importFile(String dataset, EditableLayerImpl layer, Charset charset)
      throws IOException {
    ...
    try {
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "Failed to access the shapefile at either '" + dataset + "' or '" + dataset + ".shp'",
            e);
      }
    }
    ...
    try {
      Class geometryClass =
          JTSUtilities.findBestGeometryClass(shpReader.getHeader().getShapeType());
                ...
                if (shpReader.hasNext() && dbfReader.hasNext()) {
                  ...
                  try {
                    ...
                    if (filterEnvelope == null
                        || filterEnvelope.intersects(geometry.getEnvelopeInternal())) {
                      ...
                      if (geometry.isEmpty()) {
                        ...
                        // ?
                        SpatialDatabaseRecord spatial_record =
                    ...
                    // org.geotools.data.shapefile.shp.ShapefileReader.Record.shape() can throw this
                    // exception
  ...
  private CoordinateReferenceSystem readCRS(ShpFiles shpFiles, ShapefileReader shpReader) {
    try {
      PrjFileReader prjReader =
          new PrjFileReader(shpFiles.getReadChannel(ShpFileType.PRJ, shpReader));
      try {
