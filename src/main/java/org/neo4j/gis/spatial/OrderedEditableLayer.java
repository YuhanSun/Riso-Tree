...
/**
 * This class extends the EditableLayerImpl in a way that allows for the geometry order to be
 * maintained. If the user wishes to iterate through the geometries in the same order they were
 * created, they can use the getAllGeometryNodes method for this. The super-class EditableLayerImpl
 * used to have this behavior, but the cost of maintaining the chain through the REST API was too
 * high, because the previous node could not be easily cached. So we moved it to this class, and
 * made it optional. The Java API should not suffer the performance penalty of this, but we decided
 * to make the default behavior non-ordered for a simpler data structure.
 * 
 * @author craig
 */
public class OrderedEditableLayer extends EditableLayerImpl {
  ...
  protected Node addGeomNode(Geometry geom, String[] fieldsName, Object[] fields) {
    ...
    if (previousGeomNode == null) {
      TraversalDescription traversalDescription =
          getDatabase().traversalDescription().order(BranchOrderingPolicies.POSTORDER_BREADTH_FIRST)
      ...
      for (Node node : createTraverserInBackwardsCompatibleWay(traversalDescription, layerNode)
          .nodes()) {
  ...
  /**
   * Provides a method for iterating over all nodes that represent geometries in this dataset. This
   * is similar to the getAllNodes() methods from GraphDatabaseService but will only return nodes
   * that this dataset considers its own, and can be passed to the GeometryEncoder to generate a
