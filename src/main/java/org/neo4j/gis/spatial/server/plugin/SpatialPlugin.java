...
public class SpatialPlugin extends ServerPlugin {
  ...
  public Iterable<Node> addSimplePointLayer(@Source GraphDatabaseService db,
      @Description("The layer to find or create.") @Parameter(name = "layer") String layer,
      @Description("The node property that contains the latitude. Default is 'lat'") @Parameter(
          name = "lat", optional = true) String lat,
      @Description("The node property that contains the longitude. Default is 'lon'") @Parameter(
          name = "lon", optional = true) String lon) {
  ...
  public Iterable<Node> addEditableLayer(@Source GraphDatabaseService db,
      @Description("The layer to find or create.") @Parameter(name = "layer") String layer,
      @Description("The format for internal representation, either WKB or WKT") @Parameter(
          name = "format") String format,
      @Description("The name of the node property carrying the spatial geometry info") @Parameter(
          name = "nodePropertyName") String nodePropertyName) {
    System.out.println("Creating new layer '" + layer + "' unless it already exists");
    SpatialDatabaseService spatialService = getSpatialDatabaseService(db);
    return singleton(
        spatialService.getOrCreateEditableLayer(layer, format, nodePropertyName).getLayerNode());
  }

  @PluginTarget(GraphDatabaseService.class)
  @Description("add a new dynamic layer exposing a filtered view of an existing layer")
  public Iterable<Node> addCQLDynamicLayer(@Source GraphDatabaseService db,
      @Description("The master layer to find") @Parameter(
          name = "master_layer") String master_layer,
      @Description("The name for the new dynamic layer") @Parameter(name = "name") String name,
      @Description("The type of geometry to use for streaming data from the new view") @Parameter(
          name = "geometry", optional = true) String geometry,
      @Description("The CQL query to use for defining this dynamic layer") @Parameter(
          name = "layer") String query) {
    System.out.println(
        "Creating new dynamic layer '" + name + "' from existing layer '" + master_layer + "'");
    SpatialDatabaseService spatialService = getSpatialDatabaseService(db);
    DynamicLayer dynamicLayer =
        spatialService.asDynamicLayer(spatialService.getLayer(master_layer));
  ...
  public Iterable<Node> addNodeToLayer(@Source GraphDatabaseService db,
      @Description("The node representing a geometry to add to the layer") @Parameter(
          name = "node") Node node,
  ...
  public Iterable<Node> addNodesToLayer(@Source GraphDatabaseService db,
      @Description("The nodes representing geometries to add to the layer") @Parameter(
  ...
  public Iterable<Node> addGeometryWKTToLayer(@Source GraphDatabaseService db,
      @Description("The geometry in WKT to add to the layer") @Parameter(
  ...
  public Iterable<Node> findGeometriesInBBox(@Source GraphDatabaseService db,
      ...
      @Description("The layer to search. Can be a dynamic layer with pre-defined CQL filter.") @Parameter(
    ...
    try (Transaction tx = db.beginTx()) {
      ...
      List<Node> result = GeoPipeline
          .startWithinSearch(layer,
              layer.getGeometryFactory().toGeometry(new Envelope(minx, maxx, miny, maxy)))
          .toNodeList();
      tx.success();
      return result;
    }
  ...
  public Iterable<Node> findGeometriesIntersectingBBox(@Source GraphDatabaseService db,
      @Description("The minimum x value of the bounding box") @Parameter(name = "minx") double minx,
      @Description("The maximum x value of the bounding box") @Parameter(name = "maxx") double maxx,
      @Description("The minimum y value of the bounding box") @Parameter(name = "miny") double miny,
      @Description("The maximum y value of the bounding box") @Parameter(name = "maxy") double maxy,
    ...
    try (Transaction tx = db.beginTx()) {
      ...
      List<Node> result = GeoPipeline
          .startIntersectWindowSearch(layer, new Envelope(minx, maxx, miny, maxy)).toNodeList();
      tx.success();
      return result;
    }
  }
  ...
  public Iterable<Node> findClosestGeometries(@Source GraphDatabaseService db,
      ...
      @Description("The maximum distance in km") @Parameter(
  ...
  public Iterable<Node> findGeometriesWithinDistance(@Source GraphDatabaseService db,
      @Description("The x value of a point") @Parameter(name = "pointX") double pointX,
      @Description("The y value of a point") @Parameter(name = "pointY") double pointY,
      @Description("The distance from the point to search") @Parameter(
          name = "distanceInKm") double distanceInKm,
      @Description("The layer to search. Can be a dynamic layer with pre-defined CQL filter.") @Parameter(
          name = "layer") String layerName) {
    // System.out.println("Finding Geometries in layer '" + layerName + "'");
