...
public class Utilities {
  ...
  public static Envelope fromJtsToNeo4j(com.vividsolutions.jts.geom.Envelope envelope) {
    return new Envelope(envelope.getMinX(), envelope.getMaxX(), envelope.getMinY(),
        envelope.getMaxY());
  }

  public static com.vividsolutions.jts.geom.Envelope fromNeo4jToJts(Envelope envelope) {
    return new com.vividsolutions.jts.geom.Envelope(envelope.getMinX(), envelope.getMaxX(),
  ...
  private static org.neo4j.gis.spatial.rtree.Envelope extractEnvelopeFromFilter(Filter filter,
    ...
    } else if (filter instanceof IntersectsImpl || filter instanceof ContainsImpl
        || filter instanceof CrossesImpl || filter instanceof EqualsImpl
  ...
  private static Envelope extractEnvelopeFromGeometryFilter(GeometryFilterImpl intersectFilter) {
    if (intersectFilter.getExpression1() instanceof LiteralExpressionImpl) {
      return extractEnvelopeFromLiteralExpression(
          (LiteralExpressionImpl) intersectFilter.getExpression1());
    } else if (intersectFilter.getExpression2() instanceof LiteralExpressionImpl) {
      return extractEnvelopeFromLiteralExpression(
          (LiteralExpressionImpl) intersectFilter.getExpression2());
    }
  ...
  public static class ComparatorOnXMin implements Comparator<Node> {
    ...
    public int compare(Node o1, Node o2) {
      return Double.compare(decoder.decodeEnvelope(o1).getMinX(),
          decoder.decodeEnvelope(o2).getMinX());
    }
  }
  ...
  public static class ComparatorOnYMin implements Comparator<Node> {
    ...
    @Override
    public int compare(Node o1, Node o2) {
      return Double.compare(decoder.decodeEnvelope(o1).getMinY(),
          decoder.decodeEnvelope(o2).getMinY());
    }
  }
