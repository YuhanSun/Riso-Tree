...
public class SpatialDatabaseService implements Constants {
  ...
  public String[] getLayerNames() {
    ...
    try (Transaction tx = getDatabase().beginTx()) {
      for (Relationship relationship : getSpatialRoot()
          .getRelationships(SpatialRelationshipTypes.LAYER, Direction.OUTGOING)) {
  ...
  /**
   * Convert a layer into a DynamicLayer. This will expose the ability to add views, or 'dynamic
   * layers' to the layer.
   * 
   * @param layer
  ...
  public EditableLayer getOrCreatePointLayer(String name, String xProperty, String yProperty) {
    ...
    if (layer == null) {
      ...
      return (EditableLayer) createLayer(name, SimplePointEncoder.class, SimplePointLayer.class,
          encoderConfig);
  ...
  public Layer getOrCreateLayer(String name, Class<? extends GeometryEncoder> geometryEncoder,
  ...
  /**
   * This method will find the Layer when given a geometry node that this layer contains. This
   * method used to make use of knowledge of the RTree, traversing backwards up the tree to find the
   * layer node, which is fast. However, for reasons of clean abstraction, this has been refactored
   * to delegate the logic to the layer, so that each layer can do this in an implementation
   * specific way. Now we simply iterate through the layers datasets and the first one to return
   * true on the SpatialDataset.containsGeometryNode(Node) method is returned.
   * 
   * We can consider removing this method for a few reasons: * It is non-deterministic if more than
   * one layer contains the same geometry * None of the current code appears to use this method
   * 
  ...
  public SimplePointLayer createSimplePointLayer(String name, String xProperty, String yProperty,
    ...
    return (SimplePointLayer) createLayer(name, SimplePointEncoder.class, SimplePointLayer.class,
        makeConfig(xProperty, yProperty, bboxProperty),
  ...
  public Layer createLayer(String name, Class<? extends GeometryEncoder> geometryEncoderClass,
      Class<? extends Layer> layerClass, String encoderConfig) {
    return createLayer(name, geometryEncoderClass, layerClass, encoderConfig, null);
  }

  public Layer createLayer(String name, Class<? extends GeometryEncoder> geometryEncoderClass,
      Class<? extends Layer> layerClass, String encoderConfig, CoordinateReferenceSystem crs) {
    Transaction tx = database.beginTx();
    try {
  ...
  public static int convertGeometryNameToType(String geometryName) {
    ...
    try {
      return convertJtsClassToGeometryType(
          (Class<? extends Geometry>) Class.forName("com.vividsolutions.jts.geom." + geometryName));
  ...
  public static String convertGeometryTypeToName(Integer geometryType) {
    return convertGeometryTypeToJtsClass(geometryType).getName()
        .replace("com.vividsolutions.jts.geom.", "");
  }
  ...
  /**
   * Create a new layer from the results of a previous query. This actually copies the resulting
   * geometries and their attributes into entirely new geometries using WKBGeometryEncoder. This
   * means it is independent of the format of the original data. As a consequence it will have lost
   * any domain specific capabilities of the original graph, if any. Use it only if you want a copy
   * of the geometries themselves, and nothing more. One common use case would be to create a
   * temporary layer of the results of a query than you wish to now export to a format that only
   * supports geometries, like Shapefile, or the PNG images produced by the ImageExporter.
   * 
   * @param layerName
   * @param results
  ...
  /**
   * Support mapping a String (ex: 'SimplePoint') to the respective GeometryEncoder and Layer
   * classes to allow for more streamlined method for creating Layers This was added to help support
   * Spatial Cypher project.
   */
  public static class RegisteredLayerType {
    ...
    public RegisteredLayerType(String typeName, Class<? extends GeometryEncoder> geometryEncoder,
        Class<? extends Layer> layerClass, org.geotools.referencing.crs.AbstractCRS crs,
        String defaultConfig) {
  ...
  static {
    registeredLayerTypes.put("SimplePoint",
        new RegisteredLayerType("SimplePoint", SimplePointEncoder.class, SimplePointLayer.class,
            org.geotools.referencing.crs.DefaultGeographicCRS.WGS84, "longitude:latitude"));
    registeredLayerTypes.put("WKT", new RegisteredLayerType("WKT", WKTGeometryEncoder.class,
        EditableLayerImpl.class, DefaultGeographicCRS.WGS84, "geometry"));
    registeredLayerTypes.put("WKB", new RegisteredLayerType("WKB", WKBGeometryEncoder.class,
        EditableLayerImpl.class, DefaultGeographicCRS.WGS84, "geometry"));
    registeredLayerTypes.put("OSM", new RegisteredLayerType("OSM", OSMGeometryEncoder.class,
        OSMLayer.class, DefaultGeographicCRS.WGS84, "geometry"));
  }
   ...
   * @return
   */
  public Layer getOrCreateRegisteredTypeLayer(String name, RegisteredLayerType registeredLayerType,
      String config) {
    return getOrCreateLayer(name, registeredLayerType.geometryEncoder,
        registeredLayerType.layerClass,
