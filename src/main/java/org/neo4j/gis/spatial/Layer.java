...
public interface Layer {
  ...
  /**
   * The layer is constructed from metadata in the layer node, which requires that the layer have a
   * no-argument constructor. The real initialization of the layer is then performed by calling this
   * method. The layer implementation can store the passed parameters for later use satisfying the
   * prupose of the layer API (see other Layer methods).
   * 
  ...
  /**
   * This method adds existing geometries to the layer for indexing. After this method is called the
   * geometry should be searchable.
   *
   * @param geomNode
  ...
  /**
   * This method adds existing geometries to the layer for indexing in bulk. After this method is
   * called the geometry should be searchable.
   *
   * @param geomNodes
  ...
  /**
   * Delete the entire layer, including the index. The specific layer implementation will decide if
   * this method should delete also the geometry nodes indexed by this layer. Some implementations
   * have data that only has meaning within a layer, and so will be deleted. Others are simply views
   * onto other more complex data models and deleting the geometry nodes might imply damage to the
   * model. Keep this in mind when coding implementations of the Layer.
   */
   ...
   * implement the layer should also provide appropriate GeometryEncoders for encoding and decoding
   * the geometries. This can be either as properties of a geometry node, or as sub-graphs
   * accessible from some geometry node.
   * 
  ...
  /**
   * Each layer can represent data stored in a specific coordinate refernece system, or projection.
   * 
   * @return
   */
  ...
  /**
   * Each layer can optionally provide a style to be used in rendering this layer. Return null if
   * you wish to leave this choice to the GIS. If a Style is returned, it is used. If a File is
   * returned, it is opened and assumed to contain SLD contents. If a String is returned, it is
   * assumed to contain SLD contents.
   * 
