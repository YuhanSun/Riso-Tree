 ...
 * geometry-point or a map optional default simplePointLayer should use the long form of
 * "latitude and longitude" like the spatial functions do
 */
...
public class SpatialProcedures {
  ...
  private static void populateEncoderClasses() {
    ...
    for (Class cls : new Class[] {SimplePointEncoder.class, OSMGeometryEncoder.class,
        SimplePropertyEncoder.class, WKTGeometryEncoder.class, WKBGeometryEncoder.class,
        SimpleGraphEncoder.class}) {
  ...
  public Stream<NodeResult> addSimplePointLayer(@Name("name") String name) {
    ...
    if (layer == null) {
      return streamNode(
          sdb.createLayer(name, SimplePointEncoder.class, SimplePointLayer.class).getLayerNode());
    } else {
  ...
  @PerformsWrites
  public Stream<NodeResult> addSimplePointLayer(@Name("name") String name,
      @Name("xProperty") String xProperty, @Name("yProperty") String yProperty) {
    SpatialDatabaseService sdb = wrap(db);
    Layer layer = sdb.getLayer(name);
    if (layer == null) {
      if (xProperty != null && yProperty != null) {
        ...
        return streamNode(
            sdb.createLayer(name, SimplePointEncoder.class, SimplePointLayer.class, encoderConfig)
                .getLayerNode());
      } else {
        throw new IllegalArgumentException(
            "Cannot create layer '" + name + "': Missing encoder config values: xProperty["
                + xProperty + "], yProperty[" + yProperty + "]");
      }
    } else {
      throw new IllegalArgumentException("Cannot create existing layer: " + name);
    }
  ...
  @PerformsWrites
  public Stream<NodeResult> addLayer(@Name("name") String name,
      @Name("encoder") String encoderClassName, @Name("encoderConfig") String encoderConfig) {
    SpatialDatabaseService sdb = wrap(db);
    Layer layer = sdb.getLayer(name);
    if (layer == null) {
      ...
      if (encoderClass != null) {
        return streamNode(
            sdb.createLayer(name, encoderClass, layerClass, encoderConfig).getLayerNode());
      } else {
        throw new IllegalArgumentException(
  ...
  @PerformsWrites
  public Stream<NodeResult> addLayerOfType(@Name("name") String name, @Name("type") String type,
      @Name("encoderConfig") String encoderConfig) {
    SpatialDatabaseService sdb = wrap(db);
    Layer layer = sdb.getLayer(name);
    if (layer == null) {
      ...
      if (knownTypes.containsKey(type)) {
        return streamNode(
            sdb.getOrCreateRegisteredTypeLayer(name, type, encoderConfig).getLayerNode());
      } else {
        throw new IllegalArgumentException("Cannot create layer '" + name + "': unknown type '"
            + type + "' - supported types are " + knownTypes.toString());
      }
    } else {
      throw new IllegalArgumentException("Cannot create existing layer: " + name);
    }
  ...
  public void removeLayer(@Name("name") String name) {
    wrap(db).deleteLayer(name,
        new ProgressLoggingListener("Deleting layer '" + name + "'", log.infoLogger()));
  }

  // todo do we want to return anything ? or just a count?
  @Procedure("spatial.addNode")
  @PerformsWrites
  public Stream<NodeResult> addNodeToLayer(@Name("layerName") String name,
      @Name("node") Node node) {
  ...
  @PerformsWrites
  public Stream<CountResult> addNodesToLayer(@Name("layerName") String name,
  ...
  @PerformsWrites
  public Stream<NodeResult> updateGeometryFromWKT(@Name("layerName") String name,
      @Name("geometry") String geometryWKT, @Name("geometryNodeId") long geometryNodeId) {
  ...
  @PerformsWrites
  public Stream<CountResult> importShapefile(@Name("layerName") String name,
  ...
  @PerformsWrites
  public Stream<CountResult> importShapefile(@Name("uri") String uri) throws IOException {
    return Stream.of(new CountResult(importShapefileToLayer(uri, null, 1000).size()));
  }

  private List<Node> importShapefileToLayer(String shpPath, EditableLayerImpl layer,
      int commitInterval) throws IOException {
    ...
    ShapefileImporter importer = new ShapefileImporter(db,
        new ProgressLoggingListener("Importing " + shpPath, log.debugLogger()), commitInterval);
    if (layer == null) {
  ...
  @PerformsWrites
  public Stream<CountResult> importOSM(@Name("layerName") String name, @Name("uri") String uri)
  ...
  @PerformsWrites
  public Stream<CountResult> importOSM(@Name("uri") String uri)
      throws IOException, XMLStreamException {
    return Stream.of(new CountResult(importOSMToLayer(uri, null, 1000)));
  }

  private long importOSMToLayer(String osmPath, EditableLayerImpl layer, int commitInterval)
      throws IOException, XMLStreamException {
    ...
    String layerName = (layer == null) ? osmPath.substring(osmPath.lastIndexOf(File.separator) + 1)
        : layer.getName();
    OSMImporter importer = new OSMImporter(layerName,
        new ProgressLoggingListener("Importing " + osmPath, log.debugLogger()));
  ...
  public Stream<NodeResult> findGeometriesInBBox(@Name("layerName") String name,
      @Name("min") Object min, @Name("max") Object max) {
    ...
    return GeoPipeline.startWithinSearch(layer, layer.getGeometryFactory().toGeometry(envelope))
        .stream().map(GeoPipeFlow::getGeomNode).map(NodeResult::new);
  }

  @Procedure("spatial.closest")
  @PerformsWrites // TODO FIX
  public Stream<NodeResult> findClosestGeometries(@Name("layerName") String name,
      @Name("coordinate") Object coordinate, @Name("distanceInKm") double distanceInKm) {
    Layer layer = getLayerOrThrow(name);
    GeometryFactory factory = layer.getGeometryFactory();
    Point point = factory.createPoint(toCoordinate(coordinate));
    List<SpatialTopologyUtils.PointResult> edgeResults =
        SpatialTopologyUtils.findClosestEdges(point, layer, distanceInKm);
    return edgeResults.stream().map(e -> e.getValue().getGeomNode()).map(NodeResult::new);
  }

  @Procedure("spatial.withinDistance")
  @PerformsWrites // TODO FIX
  public Stream<NodeDistanceResult> findGeometriesWithinDistance(@Name("layerName") String name,
      @Name("coordinate") Object coordinate, @Name("distanceInKm") double distanceInKm) {

    Layer layer = getLayerOrThrow(name);
    return GeoPipeline
        .startNearestNeighborLatLonSearch(layer, toCoordinate(coordinate), distanceInKm)
        .sort(DISTANCE).stream().map(r -> {
          double distance =
              r.hasProperty(DISTANCE) ? ((Number) r.getProperty(DISTANCE)).doubleValue() : -1;
  ...
  // If you need to return Geometries outside (eg. RETURN geometry), then consider
  // spatial.asExternalGeometry(geometry)
  public Stream<GeometryResult> decodeGeometry(@Name("layerName") String name,
      @Name("node") Node node) {

    Layer layer = getLayerOrThrow(name);
    return Stream.of(layer.getGeometryEncoder().decodeGeometry(node))
        .map(geom -> new GeometryResult(toCypherGeometry(layer, geom)));
  }
  ...
  // If you need to return Geometries outside (eg. RETURN geometry), then consider
  // spatial.asExternalGeometry(geometry)
  public Stream<GeometryResult> asGeometry(@Name("geometry") Object geometry) {

    return Stream.of(geometry).map(geom -> new GeometryResult(toCypherGeometry(null, geom)));
  }

  @Procedure("spatial.asExternalGeometry")
  // TODO: This method only exists (and differs from spatial.asGeometry()) because of a bug in
  // Cypher 3.0
  // Cypher will emit external geometry types but can only consume internal types. Once that bug is
  // fixed,
  // We can make both asGeometry() and asExternalGeometry() return the same public type, and
  // deprecate this procedure.
  public Stream<GeometryResult> asExternalGeometry(@Name("geometry") Object geometry) {
  ...
  @PerformsWrites // TODO FIX
  public Stream<NodeResult> findGeometriesIntersecting(@Name("layerName") String name,
      @Name("geometry") Object geometry) {

    Layer layer = getLayerOrThrow(name);
    return GeoPipeline.startIntersectSearch(layer, toJTSGeometry(layer, geometry)).stream()
        .map(GeoPipeFlow::getGeomNode).map(NodeResult::new);
  }
  ...
  public static class Neo4jGeometry implements org.neo4j.graphdb.spatial.Geometry {
    ...
    public Neo4jGeometry(String geometryType,
        List<org.neo4j.graphdb.spatial.Coordinate> coordinates, CRS crs) {
    ...
    public static String coordinateString(List<org.neo4j.graphdb.spatial.Coordinate> coordinates) {
      return coordinates.stream().map(
          c -> c.getCoordinate().stream().map(v -> v.toString()).collect(Collectors.joining(", ")))
  ...
  private org.neo4j.graphdb.spatial.Geometry toNeo4jGeometry(Layer layer, Object value) {
    ...
    if (value instanceof Geometry) {
      ...
      return new Neo4jGeometry(geometry.getGeometryType(),
          toNeo4jCoordinates(geometry.getCoordinates()), crs);
    }
    ...
    Coordinate coord = toCoordinate(latLon);
    if (coord != null)
      return new Neo4jPoint(coord.x, coord.y, crs);
    throw new RuntimeException("Can't convert " + value + " to a geometry");
  }

  private org.neo4j.cypher.internal.compiler.v3_1.Geometry makeCypherGeometry(double x, double y,
      org.neo4j.cypher.internal.compiler.v3_1.CRS crs) {
  ...
  private org.neo4j.cypher.internal.compiler.v3_1.Geometry makeCypherGeometry(Geometry geometry,
      org.neo4j.cypher.internal.compiler.v3_1.CRS crs) {
    if (geometry.getGeometryType().toLowerCase().equals("point")) {
      ...
      throw new RuntimeException(
          "Cypher only accepts POINT geometries, not " + geometry.getGeometryType());
    }
  }

  private org.neo4j.cypher.internal.compiler.v3_1.Geometry toCypherGeometry(Layer layer,
      Object value) {
    ...
    if (value instanceof org.neo4j.graphdb.spatial.Point) {
      org.neo4j.graphdb.spatial.Point point = (org.neo4j.graphdb.spatial.Point) value;
      List<Double> coord = point.getCoordinate().getCoordinate();
      return makeCypherGeometry(coord.get(0), coord.get(1),
          org.neo4j.cypher.internal.compiler.v3_1.CRS.fromSRID(point.getCRS().getCode()));
    }
    org.neo4j.cypher.internal.compiler.v3_1.CRS crs =
        org.neo4j.cypher.internal.compiler.v3_1.CRS.Cartesian();
    if (layer != null) {
      CoordinateReferenceSystem layerCRS = layer.getCoordinateReferenceSystem();
      if (layerCRS != null) {
        ReferenceIdentifier crsRef = layer.getCoordinateReferenceSystem().getName();
    ...
    if (value instanceof String) {
      GeometryFactory factory =
          (layer == null) ? new GeometryFactory() : layer.getGeometryFactory();
      WKTReader reader = new WKTReader(factory);
      try {
        Geometry geometry = reader.read((String) value);
        return makeCypherGeometry(geometry, crs);
    ...
    Coordinate coord = toCoordinate(latLon);
    if (coord != null)
      return makeCypherGeometry(coord.x, coord.y, crs);
    throw new RuntimeException("Can't convert " + value + " to a geometry");
  }
  ...
  private Coordinate toCoordinate(Object value) {
    ...
    if (value instanceof PropertyContainer) {
      return toCoordinate(
          ((PropertyContainer) value).getProperties("latitude", "longitude", "lat", "lon"));
    }
    if (value instanceof Map) {
  ...
  private Coordinate toCoordinate(Map map, String xName, String yName) {
    if (map.containsKey(xName) && map.containsKey(yName))
      return new Coordinate(((Number) map.get(xName)).doubleValue(),
          ((Number) map.get(yName)).doubleValue());
    return null;
  }
