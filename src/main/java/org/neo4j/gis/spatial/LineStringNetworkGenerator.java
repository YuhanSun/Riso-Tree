...
/**
 * Creates a Network of LineStrings. If a LineString start point or end point is equal to some other
 * LineString start point or end point, the two LineStrings are connected together with a
 * Relationship.
 */
public class LineStringNetworkGenerator {
  ...
  public void add(SpatialDatabaseRecord record) {
    ...
    if (geometry instanceof MultiLineString) {
      ...
      throw new IllegalArgumentException(
          "geometry type not supported: " + geometry.getGeometryType());
    }
  }
  ...
  protected void addEdgePoint(Node edge, Geometry edgePoint) {
    ...
    Iterator<SpatialDatabaseRecord> results =
        pointsLayer.getIndex().search(new SearchIntersect(pointsLayer, edgePoint));
    if (!results.hasNext()) {
