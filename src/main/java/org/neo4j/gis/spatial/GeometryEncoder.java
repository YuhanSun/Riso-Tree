...
public interface GeometryEncoder extends EnvelopeDecoder {
  ...
  /**
   * When accessing an existing layer, the Layer is constructed from a single node in the graph that
   * represents a layer. This node is expected to have a property containing the class name of the
   * GeometryEncoder for that layer, and it will be constructed and passed the layer using this
   * method, allowing the Layer and the GeometryEncoder to interact.
   * 
  ...
  /**
   * This method is called to store a bounding box for the geometry to the database. It should write
   * it to the container supplied. If the container is a node, it can be the root of an entire
   * sub-graph.
   *
   * @param geometry
   * @param container
  ...
  /**
   * Each geometry might have a set of associated attributes, or properties. These are seen as a map
   * of String to Object types, where the Objects should be primitives or Strings. This can be
   * encoded as properties of the geometry node itself (default behaviour), or stored in the graph
   * in some other way.
   * 
   * @param geomNode
