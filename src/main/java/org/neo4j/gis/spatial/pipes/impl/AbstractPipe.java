 ...
 * </pre>
 * 
 * If the current incoming S is not to be emitted and there are no other S objects to process and
 * emit, then throw a NoSuchElementException.
 *
...
public abstract class AbstractPipe<S, E> implements Pipe<S, E> {
  ...
  /**
   * The iterator method of Iterable is not faithful to the Java semantics of iterator(). This
   * method simply returns the pipe itself (which is an iterator) and thus, is useful only for
   * foreach iteration.
   *
