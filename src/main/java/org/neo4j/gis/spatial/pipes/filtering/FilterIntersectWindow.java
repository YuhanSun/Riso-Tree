...
public class FilterIntersectWindow extends AbstractFilterGeoPipe {
  ...
  public FilterIntersectWindow(GeometryFactory geomFactory, double xmin, double ymin, double xmax,
      double ymax) {
  ...
  protected boolean validate(GeoPipeFlow flow) {
    return envelope.intersects(flow.getEnvelope()) && envelopeGeom.intersects(flow.getGeometry());
  }
}
