...
public class GeoPipeline extends Pipeline<GeoPipeFlow, GeoPipeFlow> {
  ...
  protected static IdentityPipe<GeoPipeFlow> createStartPipe(
      final Iterator<SpatialDatabaseRecord> records) {
   ...
   * @return geoPipeline
   */
  public static GeoPipeline startEqualExactSearch(Layer layer, Geometry geometry,
      double tolerance) {
    return startIntersectWindowSearch(layer, geometry.getEnvelopeInternal())
  ...
  /**
   * Calculates the distance between Layer items nearest to the given point and the given point. The
   * search window created is based on Layer items density and it could lead to no results.
   * 
   ...
   * @return geoPipeline
   */
  public static GeoPipeline startNearestNeighborLatLonSearch(Layer layer, Coordinate point,
      int numberOfItemsToFind) {
    Envelope searchWindow = SpatialTopologyUtils.createEnvelopeForGeometryDensityEstimate(layer,
  ...
  /**
   * Extracts Layer items with a distance from the given point that is less than or equal the given
   * distance.
   * 
   * @param layer with latitude, longitude coordinates
   * @param point
  ...
  public static GeoPipeline startNearestNeighborLatLonSearch(Layer layer, Coordinate point,
    ...
    if (layer.getGeometryType() != null && layer.getGeometryType() == Constants.GTYPE_POINT) {
      pipeline = pipeline.propertyFilter("OrthodromicDistance", maxDistanceInKm,
   ...
   * @return geoPipeline
   */
  public static GeoPipeline startNearestNeighborSearch(Layer layer, Coordinate point,
      int numberOfItemsToFind) {
    Envelope searchWindow = SpatialTopologyUtils.createEnvelopeForGeometryDensityEstimate(layer,
        point, numberOfItemsToFind);
  ...
  public FeatureCollection<SimpleFeatureType, SimpleFeature> toStreamingFeatureCollection(
      final Envelope bounds) throws IOException {
    return toStreamingFeatureCollection(Neo4jFeatureBuilder.getTypeFromLayer(layer), bounds);
  }

  public FeatureCollection<SimpleFeatureType, SimpleFeature> toStreamingFeatureCollection(
      SimpleFeatureType featureType, final Envelope bounds) throws IOException {
  ...
  public FeatureCollection<SimpleFeatureType, SimpleFeature> toFeatureCollection(
    ...
    final ReferencedEnvelope refBounds =
        new ReferencedEnvelope(bounds, layer.getCoordinateReferenceSystem());

    final Neo4jFeatureBuilder featureBuilder =
        new Neo4jFeatureBuilder(featureType, Arrays.asList(layer.getExtraPropertyNames()));
    return new AbstractFeatureCollection(featureType) {
      @Override
      public int size() {
        return records.size();
  ...
  /**
   * Iterates through the pipeline content and creates a list of all the SpatialDatabaseRecord
   * found. This will empty the pipeline.
   * 
   ...
   * 
   * Warning: GeoPipeline doesn't modify SpatialDatabaseRecords thus the geometries contained aren't
   * those transformed by the pipeline but the original ones.
   */
  ...
  /**
   * Iterates through the pipeline content and creates a list of all the Nodes found. This will
   * empty the pipeline.
   * 
   * Warning: this method should *not* be used with pipes that extract many items from a single item
   * or with pipes that group many items into fewer items.
