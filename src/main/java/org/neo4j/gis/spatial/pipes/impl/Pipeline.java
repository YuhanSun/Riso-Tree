...
/**
 * A Pipeline is a linear composite of Pipes. Pipeline takes a List of Pipes and joins them
 * according to their order as specified by their location in the List. It is important to ensure
 * that the provided ordered Pipes can connect together. That is, that the output type of the n-1
 * Pipe is the same as the input type of the n Pipe. Once all provided Pipes are composed, a
 * Pipeline can be treated like any other Pipe.
 *
...
public class Pipeline<S, E> implements Pipe<S, E> {
  ...
  /**
   * Constructs a pipeline from the provided pipes. The ordered list determines how the pipes will
   * be chained together. When the pipes are chained together, the start of pipe n is the end of
   * pipe n-1.
   *
  ...
  /**
   * Constructs a pipeline from the provided pipes. The ordered array determines how the pipes will
   * be chained together. When the pipes are chained together, the start of pipe n is the end of
   * pipe n-1.
   *
  ...
  /**
   * Get the next object emitted from the pipeline. If no such object exists, then a
