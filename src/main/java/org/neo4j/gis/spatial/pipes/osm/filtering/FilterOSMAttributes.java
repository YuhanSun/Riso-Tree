...
public class FilterOSMAttributes extends AbstractGeoPipe {
  ...
  protected GeoPipeFlow process(GeoPipeFlow flow) {
    ...
    Node waysNode =
        geomNode.getSingleRelationship(OSMRelation.GEOM, Direction.INCOMING).getStartNode();
    Node tagNode =
        waysNode.getSingleRelationship(OSMRelation.TAGS, Direction.OUTGOING).getEndNode();
    if (tagNode.hasProperty(key) && comparison.compare(tagNode.getProperty(key), value)) {
      return flow;
    } else {
      return null;
    }
