...
/**
 * The standard DynamicIndexReader allows for graph traversal and property match queries written in
 * JSON. The JSON code is expected to be a match for a sub-graph and it's properties. It only
 * supports tree structures, since JSON is a tree format. The root of the JSON is the geometry node,
 * which means that queries for properties on nodes further away require traversals in the JSON. The
 * following example demonstrates a query for an OSM geometry layer, with a test of the geometry
 * type on the geometry node itself, followed by a two step traversal to the ways tag node, and then
 * a query on the tags.
 * 
 * <pre>
...
public class DynamicIndexReader extends LayerIndexReaderWrapper {
  ...
  /**
   * This method is there the real querying is done. It first tests for properties on the geometry
   * node, and then steps though the tree structure of the JSON, and a matching structure in the
   * graph, querying recursively each nodes properties on the way, as along as the JSON contains to
   * have properties to test, and traversal steps to take.
   * 
   * @param geomNode
   * @return true if the node matches the query string, or the query string is empty
   */
  ...
  private boolean stepAndQuery(Node source, JSONObject step) {
    if (step != null) {
      ...
      Relationship rel = source.getSingleRelationship(
          DynamicRelationshipType.withName(step.get("type").toString()),
  ...
  private boolean queryNodeProperties(Node node, JSONObject properties) {
    if (properties != null) {
      ...
      for (Object key : properties.keySet()) {
        ...
        // TODO: Find a better way to solve minor type mismatches (Long!=Integer) than the string
        // conversion below
        if (value == null || (match != null && !value.equals(match)
            && !value.toString().equals(match.toString()))) {
          return false;
        }
      }
    }
  ...
  private SearchFilter wrapSearchFilter(final SearchFilter filter) {
    return new SearchFilter() {
      ...
      public boolean needsToVisit(Envelope envelope) {
        return queryIndexNode(envelope) && filter.needsToVisit(envelope);
      }
