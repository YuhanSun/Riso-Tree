...
/**
 * This class enables support for CQL based dynamic layers. This means the filtering on the result
 * set is based on matches to a CQL query. Some key differences between CQL queries and JSON queries
 * are:
 * <ul>
 * <li>CQL will operate on the geometry itself, performing spatial operations, but also requiring
 * the geometry to be created from the graph. This makes it slower than the JSON approach, but
 * richer from a GIS perspective</li>
 * <li>JSON will operate on the graph itself, and so it is more specific to the data model, and not
 * at all specific to the GIS meaning of the data. This makes it faster, but more complex to develop
 * to. You really need to know your graph structure well to write a complex JSON query. For simple
