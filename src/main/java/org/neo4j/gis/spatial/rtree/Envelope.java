...
public class Envelope {
  ...
  /**
   * Creates an empty envelope with unknown dimension. Be sure to add something to this before
   * attempting to access the contents.
   */
  public Envelope() {}
  ...
  /**
   * Note that this doesn't exclude the envelope boundary. See JTS Envelope.
   */
  ...
  /**
   * Return the distance between the two envelopes on one dimension. This can return negative values
   * if the envelopes intersect on this dimension.
   * 
   * @param other
   * @param dimension
  ...
  public double overlap(Envelope other) {
    ...
    return intersection == null ? 0.0
        : smallest.isPoint() ? 1.0 : intersection.getArea() / smallest.getArea();
  }
  ...
  public Envelope intersection(Envelope other) {
    if (isValid() && other.isValid() && getDimension() == other.getDimension()) {
      ...
      throw new IllegalArgumentException(
          "Cannot calculate intersection of Envelopes with different dimensions: "
  ...
  public Envelope bbox(Envelope other) {
    if (isValid() && other.isValid() && getDimension() == other.getDimension()) {
      ...
      throw new IllegalArgumentException(
          "Cannot calculate bounding box of Envelopes with different dimensions: "
              + this.getDimension() + " != " + other.getDimension());
    }
  }
