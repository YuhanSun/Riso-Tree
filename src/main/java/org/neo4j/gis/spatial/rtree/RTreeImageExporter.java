...
public class RTreeImageExporter {
  ...
  private final Color[] colors = new Color[] {Color.BLUE, Color.CYAN, Color.GREEN, Color.RED,
  ...
  public RTreeImageExporter(Layer layer, RTreeIndex index, Coordinate min, Coordinate max) {
    initialize(layer.getGeometryFactory(), layer.getGeometryEncoder(),
        layer.getCoordinateReferenceSystem(), Neo4jFeatureBuilder.getTypeFromLayer(layer), index);
  ...
  public void initialize(GeometryFactory geometryFactory, GeometryEncoder geometryEncoder,
      CoordinateReferenceSystem crs, SimpleFeatureType featureType, RTreeIndex index) {
  ...
  public void saveRTreeLayers(File imagefile, Node rootNode, int levels) throws IOException {
    saveRTreeLayers(imagefile, rootNode, levels, new EmptyMonitor(), new ArrayList<>(),
        new ArrayList<>(), null, null);
  }

  public void saveRTreeLayers(File imagefile, Node rootNode, List<Envelope> envelopes, int levels)
      throws IOException {
    saveRTreeLayers(imagefile, rootNode, levels, new EmptyMonitor(), new ArrayList<>(), envelopes,
        null, null);
  }
  ...
  public void saveRTreeLayers(File imagefile, int levels, TreeMonitor monitor,
      List<Node> foundNodes, Coordinate min, Coordinate max) throws IOException {
    saveRTreeLayers(imagefile, index.getIndexRoot(), levels, monitor, foundNodes, new ArrayList<>(),
        min, max);
  }

  public void saveRTreeLayers(File imagefile, Node rootNode, int levels, TreeMonitor monitor,
      List<Node> foundNodes, List<Envelope> envelopes, Coordinate min, Coordinate max)
      throws IOException {
  ...
  private void drawGeometryNodes(MapContent mapContent, List<Node> nodes, Color color) {
    ...
    mapContent.addLayer(
        new org.geotools.map.FeatureLayer(makeGeometryNodeFeatures(nodes, featureType), style));
  }
  ...
  private void drawIndexNodes(int level, MapContent mapContent,
      List<RTreeIndex.NodeWithEnvelope> nodes, Color color) throws IOException {
  ...
  private void drawBounds(MapContent mapContent, ReferencedEnvelope bounds, Color color)
    ...
    mapContent.addLayer(new org.geotools.map.FeatureLayer(
        makeEnvelopeFeatures(new Coordinate(min[0], min[1]), new Coordinate(max[0], max[1])),
        style));
  }

  private MemoryFeatureCollection makeEnvelopeFeatures(List<Envelope> envelopes) {
    SimpleFeatureType featureType =
        Neo4jFeatureBuilder.getType("Polygon", Constants.GTYPE_POLYGON, crs, new String[] {});
    Neo4jFeatureBuilder featureBuilder =
        new Neo4jFeatureBuilder(featureType, new ArrayList<String>());
  ...
  private MemoryFeatureCollection makeEnvelopeFeatures(Coordinate min, Coordinate max) {
    ...
    Neo4jFeatureBuilder featureBuilder =
        new Neo4jFeatureBuilder(featureType, new ArrayList<String>());
    MemoryFeatureCollection features = new MemoryFeatureCollection(featureType);
    Coordinate[] coordinates = new Coordinate[] {new Coordinate(min.x, min.y),
        new Coordinate(min.x, max.y), new Coordinate(max.x, max.y), new Coordinate(max.x, min.y),
  ...
  private MemoryFeatureCollection makeGeometryNodeFeatures(List<Node> nodes,
      SimpleFeatureType featureType) {
    Neo4jFeatureBuilder featureBuilder =
        new Neo4jFeatureBuilder(featureType, new ArrayList<String>());
    MemoryFeatureCollection features = new MemoryFeatureCollection(featureType);
