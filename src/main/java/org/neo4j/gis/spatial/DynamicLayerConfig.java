...
public class DynamicLayerConfig implements Layer, Constants {
  ...
  /**
   * Construct a new layer config by building the database structure to support the necessary
   * configuration
   * 
   * @param name of the new dynamic layer
   * @param geometryType the geometry this layer supports
   * @param query formated query string for this dynamic layer
   */
  ...
  public void delete(Listener monitor) {
    throw new SpatialDatabaseException(
        "Cannot delete dynamic layers, delete the base layer instead");
  }
  ...
  /**
   * This method will scan the layer for property names that are actually used, and restrict the
   * layer properties to those
   */
  public void restrictLayerProperties() {
    ...
    System.out.println("Before property scan we have " + getExtraPropertyNames().length
  ...
  public LayerIndexReader getIndex() {
    if (parent.index instanceof LayerTreeIndexReader) {
      ...
      throw new SpatialDatabaseException(
          "Cannot make a DynamicLayer from a non-LayerTreeIndexReader Layer");
    }
  }
  ...
  public String getSignature() {
    ...
    return "DynamicLayer(name='" + getName() + "', config={layer='" + config.get("layer")
