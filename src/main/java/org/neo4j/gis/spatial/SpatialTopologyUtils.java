...
/**
 * This class is a temporary location for collecting a number of spatial utilities before we have
 * decided on a more complete analysis structure. Do not rely on this API remaining constant.
 * 
 * @author craig
 */
public class SpatialTopologyUtils {
  /**
   * Inner class associating points and resulting geometry records to facilitate the result set
   * returned.
   * 
   * @author craig
   */
  public static class PointResult
      implements Map.Entry<Point, SpatialDatabaseRecord>, Comparable<PointResult> {
    private Point point;
  ...
  public static List<PointResult> findClosestEdges(Point point, Layer layer) {
    return findClosestEdges(point, layer, 0.0);
  }

  public static List<PointResult> findClosestEdges(Point point, Layer layer, double distance) {
    ReferencedEnvelope env =
        new ReferencedEnvelope(Utilities.fromNeo4jToJts(layer.getIndex().getBoundingBox()),
  ...
  /**
   * Find geometries in the given layer that are closest to the given point while applying the
   * filter currently only handles point and linestrings (projecting them to a point) TODO Craig for
   * other geoms
   * 
   * @param point
   * @param layer
   * @param filter
   * @return list of point results containing the matched point on the geometry, the spatial record
   *         and the distance each
   */
  public static List<PointResult> findClosestEdges(Point point, Layer layer, Geometry filter) {
    ...
    Iterator<SpatialDatabaseRecord> records =
        layer.getIndex().search(new SearchIntersect(layer, filter));
    while (records.hasNext()) {
      ...
      if (geom instanceof LineString) {
        ...
        results
            .add(new PointResult(layer.getGeometryFactory().createPoint(snap), record, distance));
  ...
  /**
   * Create a Point located at the specified 'measure' distance along a Geometry. This is achieved
   * through using the JTS LengthIndexedLine.extractPoint(measure) method for finding the
   * coordinates at the specified measure along the geometry. It is equivalent to Oracle's
   * SDO_LRS.LOCATE_PT.
   * 
   ...
   * @param layer Layer the geometry is contained by, and is used to access the GeometryFactory for
   *        creating the Point
  ...
  /**
   * Find the coordinate at the specified 'measure' distance along a Geometry. This is achieved
   * through using the JTS LengthIndexedLine.extractPoint(measure) method for finding the
   * coordinates at the specified measure along the geometry. It is equivalent to Oracle's
   * SDO_LRS.LOCATE_PT.
   * 
  ...
  /**
   * Create a Point located at the specified 'measure' distance along a Geometry, and offset to the
   * left of the Geometry by the specified offset distance. This is achieved through using the JTS
   * LengthIndexedLine.extractPoint(measure) method for finding the coordinates at the specified
   * measure along the geometry. It is equivalent to Oracle's SDO_LRS.LOCATE_PT.
   * 
   * @see http://download.oracle.com/docs/cd/B13789_01/appdev.101/b10826/sdo_lrs_ref.htm#i85478
   * @see http://www.vividsolutions.com/jts/javadoc/com/vividsolutions/jts/linearref/LengthIndexedLine.html
   * @param layer Layer the geometry is contained by, and is used to access the GeometryFactory for
  ...
  /**
   * Find the coordinate located at the specified 'measure' distance along a Geometry, and offset to
   * the left of the Geometry by the specified offset distance. This is achieved through using the
   * JTS LengthIndexedLine.extractPoint(measure) method for finding the coordinates at the specified
   * measure along the geometry. It is equivalent to Oracle's SDO_LRS.LOCATE_PT.
   * 
   * @see http://download.oracle.com/docs/cd/B13789_01/appdev.101/b10826/sdo_lrs_ref.htm#i85478
   * @see http://www.vividsolutions.com/jts/javadoc/com/vividsolutions/jts/linearref/LengthIndexedLine.html
  ...
  /**
   * Adjust the size and position of a ReferencedEnvelope using fractions of the current size. For
   * example:
   * 
   * <pre>
   ...
   * </pre>
   * 
   * This will zoom in to show 30% of the height and width, and will also move the visible window
   * 10% to the left and 10% up.
   * 
   ...
   */
  public static ReferencedEnvelope adjustBounds(ReferencedEnvelope bounds, double zoomFactor,
      double[] offsetFactor) {
  ...
  /**
   * Create an Envelope that should approximately include the specified number of geometries, based
   * on a simple linear calculation of the geometry density. If the layer has fewer geometries, then
   * the layer bounds will be returned. If the limit is set to zero (or negative), a point Envelope
   * will be returned.
   * 
   * @param layer the layer whose geometry density is to be used to estimate the size of the
   *        envelope
   ...
   */
  public static Envelope createEnvelopeForGeometryDensityEstimate(Layer layer, Coordinate point,
      int limit) {
    if (limit < 1) {
    ...
    if (count > limit) {
      return createEnvelopeForGeometryDensityEstimate(layer, point,
