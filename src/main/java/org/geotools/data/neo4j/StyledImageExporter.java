...
public class StyledImageExporter {
  ...
  /**
   * When zooming in, it is useful to also control the location of the visable window using offsets
   * from the center, in fractions of the bounding box dimensions. Use negative values to adjust
   * left or down.
   * 
  ...
  public void saveLayerImage(String layerName, String sldFile, File imagefile,
      ReferencedEnvelope bounds, int width, int height, double zoom) throws IOException {
    setZoom(zoom);
  ...
  public void saveLayerImage(String layerName, String sldFile, File imagefile,
      ReferencedEnvelope bounds) throws IOException {
    String[] layerNames = new String[] {layerName};
    saveLayerImage(layerNames, sldFile, imagefile, bounds);
  }

  public void saveImage(FeatureCollection<SimpleFeatureType, SimpleFeature> features,
      String sldFile, File imagefile) throws IOException {
    saveImage(features, getStyleFromSLDFile(sldFile), imagefile);
  }

  public void saveImage(FeatureCollection<SimpleFeatureType, SimpleFeature> features, Style style,
  ...
  public void saveImage(FeatureCollection<SimpleFeatureType, SimpleFeature>[] features,
      Style[] styles, File imagefile, ReferencedEnvelope bounds) throws IOException {
    MapContent mapContent = new MapContent();
  ...
  public void saveLayerImage(String[] layerNames, String sldFile, File imagefile,
      ReferencedEnvelope bounds) throws IOException {
    ...
    try (Transaction tx = db.beginTx()) {
      ...
      for (int i = 0; i < layerNames.length; i++) {
        ...
        if (featureStyle == null) {
          ...
          System.out.println("Created style from geometry '"
              + featureSource.getSchema().getGeometryDescriptor().getType() + "': " + featureStyle);
        }
  ...
  private void saveMapContentToImageFile(MapContent mapContent, File imagefile,
      ReferencedEnvelope bounds) throws IOException {
   ...
   * appropriate style creating method. TODO: Consider adding support for attribute based color
   * schemes like in http://docs.geotools.org/stable/userguide/examples/stylefunctionlab.html
   */
  public static Style createStyleFromGeometry(FeatureType schema, Color strokeColor,
      Color fillColor) {
    if (schema != null) {
    ...
    style.featureTypeStyles()
        .addAll(createPolygonStyle(strokeColor, fillColor).featureTypeStyles());
   ...
   * Create a Style to draw polygon features
   */
  public static Style createPolygonStyle(Color strokeColor, Color fillColor, double stokeGamma,
      double fillGamma, int strokeWidth) {
    // create a partially opaque outline stroke
    Stroke stroke = styleFactory.createStroke(filterFactory.literal(strokeColor),
        filterFactory.literal(strokeWidth), filterFactory.literal(stokeGamma));

    // create a partial opaque fill
    Fill fill =
        styleFactory.createFill(filterFactory.literal(fillColor), filterFactory.literal(fillGamma));
    ...
    try {
      rule.setFilter(ECQL
          .toFilter("geometryType(the_geom)='Polygon' or geometryType(the_geom)='MultiPoligon'"));
  ...
  private static Style createLineStyle(Color strokeColor) {
    Stroke stroke =
        styleFactory.createStroke(filterFactory.literal(strokeColor), filterFactory.literal(1));
  ...
  private static Style createPointStyle(Color strokeColor, Color fillColor) {
    ...
    mark.setStroke(
        styleFactory.createStroke(filterFactory.literal(strokeColor), filterFactory.literal(2)));
    ...
    try {
      rule.setFilter(
          ECQL.toFilter("geometryType(the_geom)='Point' or geometryType(the_geom)='MultiPoint'"));
    } catch (CQLException e) {
      // TODO
      e.printStackTrace();
    }
  ...
  public static void main(String[] args) {
    if (args.length < 4) {
      System.err.println(
          "Too few arguments. Provide: 'database' 'exportdir' 'stylefile' zoom layer <layers..>");
      return;
    }
