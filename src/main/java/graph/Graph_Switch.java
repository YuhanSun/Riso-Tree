...
//		addLabel();
	...
	public static void createNewRelationship()
	{
		...
		try {
			...
			String graph_path = String.format("/mnt/hgfs/Ubuntu_shared/GeoMinHop/data/%s/graph.txt", dataset);
...
//				Iterable<Relationship> rels = osmNode.getRelationships(Direction.INCOMING, GraphRel.GRAPH_LINK);
...
//			}
	...
	/**
	 * remove label of osm nodes, including 
	 * on original GRAPH_0 nodes
	 * original GRAPH_0 label is retained
	 */
	...
	/**
	 * generate the map file of graph node id (R-tree version) to neo4j pos
	 * both spatial and non-spatial vertices are included
	 */
