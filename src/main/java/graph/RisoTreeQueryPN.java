...
//		}
	 ...
	 * @return
	 */
	public String formSubgraphQuery_ForSpatialFirst_Block(Query_Graph query_Graph, int limit, Explain_Or_Profile explain_Or_Profile,
			HashMap<Integer, MyRectangle> spa_predicates, int pos, ArrayList<Long> ids, 
...
//	}
	...
	/**
	 * Does not consider condition that area of query rectangle is 0.
	 * If so, such function cannot make the correct decision.
	 * @param query_Graph
	 * @param limit
	 */
	public void Query(Query_Graph query_Graph, int limit)
	{
		try {
			Utility.print("Initialize variables for query");
			range_query_time = 0;
			get_iterator_time = 0;
			iterate_time = 0;
			result_count = 0;
...
//							for ( int neighbor_id : NL_cards_vector.keySet())
//							{
//								int NL_label_size = (Integer) node.getProperty(PN_size_propertyname.get(key).get(neighbor_id));
//								NL_cards_vector.put(neighbor_id, (NL_cards_vector.get(neighbor_id) + ratio * NL_label_size));
//							}
...
//						HashMap<Integer, Double> NL_cards_vector = NL_cards.get(key);
					...
					if ( outputLevelInfo)
					{
						...
						if ( realMinPNSize < min_spa_card)
							...
							logWriteLine += "spa predicate is more selective\n";
						logWriteLine += String.format("NL_serialize time: %d\n", System.currentTimeMillis() - start1);
						logWriteLine += String.format("level %d time: %d\n", level_index, System.currentTimeMillis() - startLevel);
						Utility.print(logWriteLine);
						OwnMethods.WriteFile(logPath, true, logWriteLine + "\n");
					}
					...
					if (forceGraphFirst) {
						Utility.print("force graph first even min PN size is " + realMinPNSize);
						realMinPNSize = 0;
					}
					
					if ( realMinPNSize < min_spa_card)
					{
						Utility.print("PN is more selective");
						int index = 0;
						ArrayList<Long> id_pos_list = new ArrayList<Long>();
						double idIndex = 0;
						for ( int id : min_NL_list)
						{
							...
							if ( index == 500)
							{
								Utility.print("Executed percentage: " + idIndex/min_NL_list.size());
								String query = formSubgraphQuery(query_Graph, -1, Explain_Or_Profile.Profile, spa_predicates, min_NL_neighbor_id, id_pos_list);
								if ( outputQuery)
								{
									Utility.print(query);
								...
								if ( outputExecutionPlan)
								{
									...
									if (planDescription.hasProfilerStatistics())
										Utility.print("cypher statistics: " + planDescription.getProfilerStatistics().getDbHits());
									else
					...
					else	// if spatial predicate is more selective
					{
						...
						for ( Node node : overlap_MBR_list)
						{
							...
							range_query_time += time;	levelTime += time;

							if ( ids.size() != 0)
							{
								Utility.print("Execute percentage: " + nodeIndex / overlap_MBR_list.size());
								start = System.currentTimeMillis();
...
//		return null;
	 ...
	 * @return
	 */
	public String formSubgraphQuery_ForSpatialFirst_Block_HMBR(Query_Graph query_Graph, int limit, Explain_Or_Profile explain_Or_Profile,
			HashMap<Integer, MyRectangle> spa_predicates, int pos, ArrayList<Long> ids, 
			HashMap<Integer, Integer> NL_hopnum, Node node, int[][] min_hop_array)
	{
		String query = "";
		switch (explain_Or_Profile) {
		case Profile:
...
//			query += String.format(" and id(a%d) in %s\n", key, pos_id_list.toString());
	 ...
	 * @return
	 */
	public String formSubgraphQuery_HMBR(Query_Graph query_Graph, int limit, Explain_Or_Profile explain_Or_Profile,
			HashMap<Integer, MyRectangle> spa_predicates, int pos, ArrayList<Long> ids, int[][] min_hop_array)
	{
		String query = "";
		switch (explain_Or_Profile) {
		case Profile:
						...
						for ( Node node : overlap_MBR_list)
						{
							...
							if ( ids.size() != 0)
							{
								...
								String query = formSubgraphQuery_ForSpatialFirst_Block_HMBR(query_Graph, limit, 
										Explain_Or_Profile.Profile, spa_predicates, minSpaID,
										ids, min_hop.get(minSpaID), node, min_hop_array);
								if (outputQuery)
								{
									Utility.print(query);
									OwnMethods.WriteFile(logPath, true, query + "\n");
...
//		return false;
			...
			HashMap<Integer, HashMap<Integer, HashSet<String>>> spaPathsMap)
	{
		...
		while ( !queue.isEmpty())
		{
			...
			{
				Iterable<Relationship> rels = left.node.getRelationships(RTreeRel.RTREE_CHILD, Direction.OUTGOING);
				long start = System.currentTimeMillis();
				Iterator<Relationship> iterator = rels.iterator();
...
//			List<Long[]> idPairs = this.spatialJoinRTree(distance, pos, spaPathsMap);
			...
			Utility.print("join time: " + join_time);
			Utility.print("join count: " + idPairs.size());
