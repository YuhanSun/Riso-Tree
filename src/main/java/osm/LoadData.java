...
//				Envelope bbox = new Envelope(12.90, 13.0064074, 55.59, 55.60);
	...
	public static void LoadTest()
	{
		try
		{
			...
			BatchInserter batchInserter = BatchInserters.inserter(new File(db_path).getAbsoluteFile(), config);
			...
			((GraphDatabaseAPI) db).getDependencyResolver().resolveDependency(Procedures.class).registerProcedure(SpatialProcedures.class);

			importer.reIndex(db, 100000);
			db.shutdown();

			System.out.println("Total Time: " + (System.currentTimeMillis() - start));
			//				GraphDatabaseService graph = new GraphDatabaseFactory().newEmbeddedDatabase(new File(db_path));
	...
	public static void BuildIndex()
	{
		try {
			...
			GraphDatabaseService graph = new GraphDatabaseFactory().newEmbeddedDatabase(new File(db_path));
			((GraphDatabaseAPI) graph).getDependencyResolver().resolveDependency(Procedures.class).registerProcedure(SpatialProcedures.class);
