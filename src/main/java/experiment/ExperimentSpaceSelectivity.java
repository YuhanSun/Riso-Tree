...
public class ExperimentSpaceSelectivity {
	...
	{	
		...
		case Ubuntu:
			...
			resultDir = String.format("/mnt/hgfs/Google_Drive/Experiment_Result/Riso-Tree/SpaceSelectivity/%s", dataset);
...
//		generateQueryRectangleForSpaceSelectivityMeter();
					...
					for ( int k = 0; k < experiment_count; k++)
					{
						...
						if ( total_range.min_x < 0)
						{
							...
							write_line = String.format("%f\t%f\t%f\t%f\n", 
									centerX - offsetLon, centerY - offsetLat, centerX + offsetLon, centerY + offsetLat);
						}
						else
						{
							double sideLength = Math.sqrt((double)area);
	...
	/**
	 * Generate random position rectangles whose selectivity
	 * are based on their region area. (Angle)
	 */
		...
		try
		{
			for ( int i = 0; i < dataset_a.size(); i++)
			{
				...
				while ( selectivity < endSelectivity)
				{
					...
					{
					case Ubuntu:
						output_path = String.format("%s/spa_predicate/SpaceSelectivity/%s_%.8f.txt", queryDir, dataset, selectivity);
						break;
					case Windows:
						output_path = String.format("%s\\spa_predicate\\SpaceSelectivity\\%s_%.8f.txt", queryDir, dataset, selectivity);
						break;
					...
					{
						MyRectangle rectangle = OwnMethods.GenerateQueryRectangle(random, rect_size_x, rect_size_y, total_range);
						write_line = String.format("%f\t%f\t%f\t%f\n", 
								rectangle.min_x, rectangle.min_y, rectangle.min_y, rectangle.max_y);
						OwnMethods.WriteFile(output_path, true, write_line);
					}
					selectivity *= 10;
				}
