...
public class QueryNodeCount {
	...
	{	
		...
		for ( int id :ids)
		{
			...
			queryrect.add(new MyRectangle(entity.lon, entity.lon, entity.lat, entity.lat));
		}
	...
	public void RisoTreePN(ArrayList<Integer> nodeCountList, int query_id)
	{
		try {
			...
			switch (systemName) {
			case Ubuntu:
				result_detail_path = String.format("%s/risotree_PN%d_%d.txt", resultDir, MAX_HOPNUM, query_id);
